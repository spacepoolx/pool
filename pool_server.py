...
class PoolServer:
    ...
    async def submit_partial(self, request) -> Dict:
        ...
        res_dict = await self.pool.process_partial(partial, time_received_partial, balance, curr_difficulty)

        if "error_code" in res_dict and "error_code" == PoolErr.NOT_FOUND:
            asyncio.create_task(await_and_call(self.pool.process_partial, partial, time_received_partial, balance, curr_difficulty))
