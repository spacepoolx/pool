...
class Pool:
    def __init__(self, private_key: PrivateKey, config: Dict, constants: ConsensusConstants):
        ...
        # TODO: potentially teak these numbers for security and performance
        self.pool_url = "https://spacepoolx.com"
        ...
        self.default_pool_puzzle_hash: bytes32 = decode_puzzle_hash("xch12ma5m7sezasgh95wkyr8470ngryec27jxcvxcmsmc4ghy7c4njssnn623q")
        self.pool_fee_puzzle_hash: bytes32 = decode_puzzle_hash("txch1h8ggpvqzhrquuchquk7s970cy0m0e0yxd4hxqwzqkpzxk9jx9nzqmd67ux")
        ...
        self.payment_loop: Optional[asyncio.Task] = None

        self.max_additions_per_transaction = 500

        # This is the list of payments that we have not sent yet, to farmers
        self.pending_payments: List[List[Dict]] = []

    async def start(self):
        self.store = await PoolStore.create()
        self.pending_point_partials = asyncio.Queue()
        self.full_node_client = await FullNodeRpcClient.create(self.config["self_hostname"], uint16(8555), DEFAULT_ROOT_PATH, self.config)
        self_hostname = self.config["self_hostname"]
        self.node_rpc_client = await FullNodeRpcClient.create(self_hostname, uint16(8555), DEFAULT_ROOT_PATH, self.config)
    ...
    async def collect_pool_rewards_loop(self):
        ...
        while True:
            try:
                ...
                coin_records: List[CoinRecord] = await self.node_rpc_client.get_coin_records_by_puzzle_hashes(scan_phs, include_spent_coins=False, end_height=peak_height - self.confirmation_security_threshold)
                ...
                for cr in coin_records:
                    ...
                    farmer_records = await self.store.get_farmer_records_for_p2_singleton_phs(set([cr.coin.puzzle_hash for cr in coin_records]))
        ...
        while True:
            try:
                ...
                for coin_record in coin_records:
                    ...
                    if (coin_record.spent_block_index > self.blockchain_state["peak"].height - self.confirmation_security_threshold):
                        continue
                ...
                blockchain_fee = 0.001 * (10 ** 12)
                amount_to_distribute = total_amount_claimed - pool_coin_amount - blockchain_fee
                ...
                async with self.store.lock:
                    ...
                    for points, ph in points_and_ph:
                        ...
                        if len(additions) == self.max_additions_per_transaction:
                            additions.append(additions_sub_list.copy())
                            additions_sub_list = []

                    # Subtract the points from each farmer
                    await self.store.clear_farmer_points()

                    # Create the payments and add to payment queue
                    # TODO: persist this to the DB
                    self.pending_payments = additions.copy()

                for payment in self.pending_payments:
                    # TODO: initiate payment in wallet
                    pass

                self.log.info(f"Sending payments: {additions}")
    ...
    async def create_absorb_transaction(self, farmer_record: FarmerRecord, singleton_coin: Coin, reward_coin_records: List[CoinRecord]) -> SpendBundle:
        ...
        try:
            ...
            async with self.store.lock:
                ...
                if farmer_record is None:
                    ...
                    assert partial.payload.proof_of_space.pool_contract_puzzle_hash == farmer_record.p2_singleton_puzzle_hash
    ...
    async def calculate_p2_singleton_ph(partial: SubmitPartial) -> bytes32:
        p2_singleton_full = P2_SINGLETON_MOD.curry(singleton_mod_has, Program.to(singleton_mod_has).get_tree_hash(), partial.payload.singleton_genesis)
        return p2_singleton_full.get_tree_hash()

    async def process_partial(self, partial: SubmitPartial, time_received_partial: uint64, balance: uint64, curr_difficulty: uint64) -> Dict:
        ...
        if time_received_partial - node_time_received_sp > self.partial_time_limit:
            return {
                ...
                "error_message": f"Received partial in {time_received_partial - node_time_received_sp}. "
                                 ...
                                 f"Response"
                                 f"must happen in less than {self.partial_time_limit} seconds. NAS or networking "
                                 f"farming can be an"
                                 f"issue",
                "points_balance": balance,
                "curr_difficulty": curr_difficulty,
